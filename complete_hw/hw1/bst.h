...
// Helper function to interleave with zeross
...
inline uint16_t code( const Point& p ) {    
    ...
    // Inline function,
    // YOUR CODE HERE 
    // moves p.y bits to odd positions, p.x remains in even
    return (interleave_with_zeros(p.y) << 1) | interleave_with_zeros(p.x);
    //return 0u;
}
...
//
// If point does not exist in the tree, does nothing.
//
